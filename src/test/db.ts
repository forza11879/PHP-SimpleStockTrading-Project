 ...
 * over these modules. Their wiring is verified by reading plus the type
 * checker, not by mocking the framework. The database is an in-memory
 * SQLite seeded like production (see `setup.ts`); the Yahoo HTTP boundary
 * is stubbed (injected fetch or raw response fixtures) rather than hit for
 * real.
 *
